...
describe('Testing github service', () => {
  ...
  describe('getMostStarred()', () => {
    it('should return item list from github api', async () => {
      ...
      const { result, limits } = await githubService.getMostStarred(new Date('2019-01-01'), 'python')
      expect(result).toHaveProperty('incomplete_results', true)
      expect(result).toHaveProperty('items')
      expect(result.items.length).toBe(30)
      expect(limits).toHaveProperty('limit', '30')
      expect(limits).toHaveProperty('remain', '29')
      expect(limits).toHaveProperty('time')
    })
  })
})
